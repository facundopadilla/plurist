...
test.describe("Manual compare selection (Task 12)", () => {
  test("editor navigates to /posts and sees the compose page", async ({ page }) => {
  ...
  test("submit-for-approval button is disabled until a variant is selected", async ({ page }) => {
    await loginAsEditor(page);
    await page.goto("/posts");

    // The submit-for-approval button should be present but disabled when no variant is selected.
    const submitBtn = page.getByTestId("submit-for-approval");
  ...
  test("editor creates a draft post via API and the post is retrievable", async ({ page }) => {
    await loginAsEditor(page);
    ...
    const created = (await createResponse.json()) as { id: number; title: string; status: string };
    expect(created.title).toBe("E2E draft post");
    expect(created.status).toBe("draft");

    const getResponse = await page.request.get(`/api/v1/posts/${created.id}`, {
      headers: {
        "X-CSRF-Token": csrf,
      },
    });
    ...
    const fetched = (await getResponse.json()) as { id: number; status: string };
