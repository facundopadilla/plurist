...
| ------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------- |
| Encryption mechanism           | Reuse `cryptography.Fernet` via a new `AIKeyVault` class sharing `_build_fernet()` pattern with a distinct PBKDF2 salt (`b"plurist-ai-keys"`) | Store keys as plaintext; use Django's `SECRET_KEY` directly        | Fernet is already a project dependency. Separate salt isolates AI keys from social tokens — compromise of one doesn't affect the other. |
...
| `backend/apps/workspace/crypto.py`                                  | Create | `AIKeyVault` — `encrypt(str) → str`, `decrypt(str) → str` using Fernet with `b"plurist-ai-keys"` salt     |
